...
from mmdet3d.models.depth_midas.backbones.beit import (
    _make_pretrained_beitl16_512,
    _make_pretrained_beitl16_384,
    _make_pretrained_beitb16_384,
    forward_beit,
)
from mmdet3d.models.depth_midas.backbones.swin_common import (
    forward_swin,
)
...
# )
from mmdet3d.models.depth_midas.backbones.swin import (
...
class DepthHead(nn.Module):
    def __init__(self, in_features, out_features, mlp_dim=128):
        ...
        self._low_res = nn.Sequential(
            nn.Conv2d(1, mlp_dim // 4, 1, 1, 0),
            nn.BatchNorm2d(mlp_dim // 4),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(mlp_dim // 4, mlp_dim, 3, 1, 1),
        ...
        self._net = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(mlp_dim, mlp_dim, 3, 1, 1),
            nn.BatchNorm2d(mlp_dim),
            nn.ReLU(inplace=True),
        )

        self.out_layer = nn.Sequential(
            nn.Conv2d(mlp_dim, mlp_dim // 2, 3, 1, 1),
            nn.BatchNorm2d(mlp_dim // 2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),
            nn.Conv2d(mlp_dim // 2, 1, 1, 1, 0),
        )

        # self._init_weight()

    def forward(self, x, lowres_d):

        x_feat = self._net(x)
        lowres_d = self._low_res(lowres_d)
        out = self.out_layer(x_feat + lowres_d)
...
class DepthLogitHead(nn.Module):
    def __init__(self, in_features, out_features, mlp_dim=128):
        """ Projector MLP
    ...
    def forward(self, bin_centers, x):
