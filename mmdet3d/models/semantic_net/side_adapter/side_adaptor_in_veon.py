...
from ..layers import MLP, MLP_3D, build_fusion_layer
...
def build_side_adapter_network_in_veon(cfg, input_shape):
...
class RegionwiseSideAdapterNetwork(nn.Module):
    ...
    def forward(
        ...
        out_features, san_features = self.forward_features(image, clip_features)
        mask_preds, attn_biases = self.decode_masks(out_features)
        return mask_preds, attn_biases, san_features
    ...
    def forward_features(
        ...
        for i, blk in enumerate(self.vit_model.blocks, start=1):
            ...
            # Record SAN features
            san_feats.append(x[:, -L:, ...].permute(0, 2, 1).reshape(x.shape[0], x.shape[-1], h, w).contiguous())
            if i < len(self.vit_model.blocks):
                x = x + pos_embed

        return outs, san_feats
