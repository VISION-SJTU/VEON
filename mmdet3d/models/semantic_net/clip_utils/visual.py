...
class RecWithAttnbiasHead(nn.Module):
    ...
    def forward(self, features, attn_bias, normalize: bool = False, clip_outputs = None):
        ...
        if self.cross_attn:
            for i, resblock in enumerate(self.resblocks):
                if self.cross_attn:
                    ...
                    if clip_outputs is None and i < len(self.resblocks) - 1:
                        x = resblock(x)
                    elif clip_outputs is not None:
                        x = resblock(x)
                        clip_outputs.save(i + self.first_layer_idx + 1, x)
        else:
            x = torch.cat([sos_token, x], dim=0)
            for i, resblock in enumerate(self.resblocks):
                x = resblock(x, attn_mask=attn_biases[i])
                if clip_outputs is not None:
                    clip_outputs.save(i + self.first_layer_idx + 1, x[-self.sos_token_num :])
        ...
        if clip_outputs is not None:
            clip_outputs['clip_feat_proj'] = \
                torch.einsum("bchw,cd->bdhw", clip_outputs[len(self.resblocks) + self.first_layer_idx], self.proj)
            return sos_token, clip_outputs
        return sos_token
    ...
    def update_remaining_clip_feats(self, clip_outputs, offsets=None, attns=None):
        start_idx = self.first_layer_idx
        cls_token = clip_outputs['{}_cls_token'.format(start_idx)]
        x = clip_outputs[start_idx]
        x = x.reshape(x.shape[0], x.shape[1], -1).permute(2, 0, 1)
        x = torch.cat([cls_token, x], dim=0)
        for i, resblock in enumerate(self.resblocks, start=start_idx + 1):
            idx = i - start_idx - 1
            if offsets is not None:
                if idx == 0:
                    x[1:, :, :] = x[1:, :, :] + offsets[0, :, :, :].permute(1, 0, 2)
                if idx == len(self.resblocks) // 2:
                    x[1:, :, :] = x[1:, :, :] + offsets[1, :, :, :].permute(1, 0, 2)
            if attns is not None:
                new_attn = self.build_attn_bias(attns[idx])
                x = resblock(x, attn_mask=new_attn)
            else:
                x = resblock(x)
            clip_outputs.save(i, x)
        clip_outputs['clip_feat_proj'] = torch.einsum("bchw,cd->bdhw", clip_outputs[len(self.resblocks) + start_idx], self.proj)
        return clip_outputs

    def build_attn_bias(self, attn):
        B, num_heads, L, _ = attn.shape
        new_attn = torch.zeros((B, num_heads, L + 1, L + 1), device=attn.device)
        new_attn[:, :, 1:, 1:] = attn
        # new_attn[:, :, 0, 0] = 0
        return new_attn.reshape(B * num_heads, L + 1, L + 1)
